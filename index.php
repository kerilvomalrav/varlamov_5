...
$uid=$_SESSION['uid'];
$res=$db->query("SELECT name,email,date,bio,radio-group-1,radio-group-2 FROM application_2 WHERE id=$uid");
foreach($res as $ell){
  ...
  $values['radio-group-1']=strip_tags($ell['radio-group-1']);
  $values['radio-group-2']=strip_tags($ell['radio-group-2']);
...
$db = new PDO('mysql:host=localhost;dbname=u47744', $user, $pass, array(PDO::ATTR_PERSISTENT => true));
  if (!empty($_COOKIE[session_name()]) &&
      session_start() && !empty($_SESSION['login'])) {
        $uid=$_SESSION['uid'];
        $stmt=$db->prepare("UPDATE application_2 SET name=?,email=?,date=?,bio=?,radio-group-1=?,radio-group-2=? WHERE id=$uid");
        $stmt->execute([$_POST['name'],$_POST['email'],$_POST['date'],$_POST['bio'],$_POST['radio-group-1'],$_POST['radio-group-2']]);
...
try {
  $stmt = $db->prepare("INSERT INTO application_2 SET name = ?, email = ?, data = ?, bio = ?, radio-group-1 = ?, radio-group-2 = ?");
  $stmt -> execute([$_POST['name'],$_POST['email'],$_POST['date'],$_POST['bio'],$_POST['radio-group-1'],$_POST['radio-group-2']]);
  $stmt = $db->prepare("INSERT INTO ultimate SET user_id = ?, nomer = ?");
  $id = $db->lastInsertId();
